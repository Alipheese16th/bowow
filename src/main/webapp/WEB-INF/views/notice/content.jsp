		...
		<h1 class="heading"><a href="${conPath}/notice/list.do" class="text-decoration-none text-black">NOTICE</a></h1>
			...
			<div class="content">
		    	...
		    	<c:if test="${empty member}">
		    		<h4>댓글 작성 권한이 없습니다 <button class="btn btn-myColor gologin">로그인</button></h4>
		    	</c:if>
		    		...
		    		<div class="card mt-2">
					  ...
					  <div class="card-body">
					    <form action="${conPath}/notice/ncWrite.do" method="post">
					    	...
					    	<input type="submit" class="btn btn-myColor mt-3" value="댓글 등록">
					    </form>
					  </div>
					</div>
		    	</c:if>
		    	...
		    	<div class="my-3">
		    		<button type="button" class="btn btn-outline-myColor" onclick="location='${conPath}/notice/list.do'">전체글 보기</button>
		    	</div>
...
// 댓글 수정버튼 눌렀을시 ajax로 댓글정보 가져오는 수정form 요청
...
// 댓글 삭제버튼
function commentDelete(ncNum) {
	if (confirm("댓글 삭제를 진행하시겠습니까?")) {
	location.href='${conPath}/notice/ncDelete.do?noticeNum=${notice.noticeNum}&ncNum='+ncNum+'&commentPageNum=${commentPaging.currentPage}&pageNum=${param.pageNum}&search=${param.search}&type=${param.type}';
	}
}
