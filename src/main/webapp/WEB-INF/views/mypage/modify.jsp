...
<head>
	...
	<script>
		...
		$(document).ready(function(){
			...
			var memberId, oldMemberPw, memberName, memberPwchk, memberEmail, memberTel;
			...
			$("#oldPw").keyup(function(){
				oldMemberPw = $(this).val();
				if(!oldMemberPw){
					$(".oldMpwResult").html("<p style='color:red;'>필수 정보입니다.</p>");
				}else if(!oldMemberPw.match(patternPw) || oldMemberPw.match(patternSpace)){
					$(".oldMpwResult").html("<p style='color:red;'>8~16자 영문 대 소문자, 숫자, 특수문자를 사용하세요.</p>");
				}else{
					$(".oldMpwResult").html("");
				};
			}); //memberPw keyup 이벤트
			$("#pw").keyup(function(){
				memberPw = $(this).val();
				if(!memberPw&& (!memberPw.match(patternPw) || memberPw.match(patternSpace))){
			...
			$("form").submit(function(){
				...
				var oldMpwResult = $(".oldMpwResult").text().trim();
				var mpwResult = $(".mpwResult").text().trim();
				var mtelResult = $(".mtelResult").text().trim();
				var memailResult = $(".memailResult").text().trim();
				var mtelResult = $(".mtelResult").text().trim();
				if(!oldMemberPw || !oldMpwResult==""){
					alert("현재 비밀번호를 확인하세요.");
					$("#oldPw").focus();
					return false;
				}else if(oldMemberPw!="${member.memberPw}"){
					alert("현재 비밀번호를 확인하세요.")
					$("#oldPw").focus();
					return false;
				}else if(mpwResult=="8~16자 영문 대 소문자, 숫자, 특수문자를 사용하세요."){
					alert("새 비밀번호를 확인하세요.")
					$("#pw").focus();
					return false;
				}
					...
					$("#tel").focus();
					return false;
				}else if(memailResult=="중복된 이메일입니다. 사용불가합니다." || memailResult=="필수 정보입니다." || memailResult=="이메일 형식으로 입력해 주세요."){
			...
			$("button[type=button]").click(function(){
				if(!oldMemberPw){
					...
					location.href='${conPath}/mypage/withdrawal.do';
				}
			});
		});
	</script>
...
<body>
	<c:if test="${empty member}">
		<script>
			alert("로그인 후 이용해주세요.");
			location.href = '${conPath}/login.do?&after=mypage/modify.do';
		</script>
	</c:if>
		...
		<div class="mypage-content-right">
			...
			<div id="content-form">
				<div class="join-wrap">
					...
					<div class="join-form">
						<form action="${conPath}/mypage/modify.do" method="post" autocomplete="off">
						<input type="hidden" name="memberId" value="${member.memberId}">
							<div class="join-row">
								<div class="join-form-wrap">
									<div class="join-form-title"><label for="id">아이디</label></div>
									<div class="join-form-text">
										<input type="text" id="id" class="focusB" placeholder="아이디를 입력해주세요." value="${member.memberId }" readonly="readonly">
										<div class="midResult bottom"></div>
									</div>
									<div class="join-form-btn"></div>
								</div>
								<div class="join-form-wrap">
									<div class="join-form-title"><label for="oldPw">현재 비밀번호</label></div>
									<div class="join-form-text">
										<input type="password" name="oldMemberPw" id="oldPw" class="focusB" placeholder="비밀번호를 입력해주세요.">
										<div class="oldMpwResult bottom"></div>
									</div>
									<div class="join-form-btn"></div>
								</div>
								<div class="join-form-wrap">
									<div class="join-form-title"><label for="pw">새 비밀번호</label></div>
									<div class="join-form-text">
										<input type="password" name="memberPw" id="pw" class="focusB" placeholder="새 비밀번호를 입력해 주세요.">
										<div class="mpwResult bottom"></div>
									</div>
									<div class="join-form-btn"></div>
								</div>
								<div class="join-form-wrap">
									<div class="join-form-title"><label for="name">이름</label></div>
									<div class="join-form-text">
								...
								<div class="join-form-wrap">
									<div class="join-form-title"><label for="email">이메일</label></div>
									<div class="join-form-text">
								...
								<div class="join-form-wrap">
									...
									<div class="join-form-text">
										<input type="text" name="memberPost" id="sample6_postcode" placeholder="우편번호" value="${member.memberPost}">
									</div>
									<div class="join-form-btn">
								...
								<div class="join-form-wrap">
									...
									<div class="join-form-text" style="margin-bottom: 45px;">
											<input type="text" name="memberAddr1" id="sample6_address" placeholder="주소" style="margin-bottom: 5px;" value="${member.memberAddr1}"><br>
											<input type="text" name="memberAddr2" id="sample6_detailAddress" placeholder="상세주소" value="${member.memberAddr2}">
									</div>
									<div class="join-form-btn"></div>
								</div>
								<div class="join-form-wrap">
									...
									<div class="mbirth-line">
										<div class="join-form-text">
											<input type="text" name="memberBirthTemp" id="datepicker" value="${member.memberBirth}">
										</div>
										<div class="join-form-btn"></div>
									</div>
								</div>
