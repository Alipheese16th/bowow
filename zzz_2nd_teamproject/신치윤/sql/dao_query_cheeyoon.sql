...
SELECT * FROM
  (SELECT ROWNUM RN, N.* FROM 
    (SELECT * FROM NOTICE 
      WHERE NOTICETITLE LIKE '%' || TRIM(' 제목 ') || '%' 
        OR NOTICECONTENT LIKE '%' || TRIM(' 제목 ') || '%' 
        OR ADMINID LIKE '%' || TRIM(' 제목 ') || '%' 
...
SELECT * FROM
  (SELECT ROWNUM RN, N.* FROM 
    (SELECT * FROM NOTICE WHERE NOTICETITLE LIKE '%' || TRIM(' 제목 ') || '%' ORDER BY NOTICEDATE DESC) N)
  WHERE RN BETWEEN 2 AND 3;
-- 제목검색 갯수 (페이징)
SELECT COUNT(*) FROM NOTICE WHERE NOTICETITLE LIKE '%' || TRIM(' 제목 ') || '%';
-- NOTICE 검색 내용
SELECT * FROM
  (SELECT ROWNUM RN, N.* FROM 
    (SELECT * FROM NOTICE WHERE NOTICECONTENT LIKE '%' || TRIM(' 내용 ') || '%' ORDER BY NOTICEDATE DESC) N)
  WHERE RN BETWEEN 2 AND 3;
-- 내용검색 갯수 (페이징)
SELECT COUNT(*) FROM NOTICE WHERE NOTICECONTENT LIKE '%' || TRIM(' 내용 ') || '%';
-- NOTICE 검색 작성자
SELECT * FROM
  (SELECT ROWNUM RN, N.* FROM 
    (SELECT * FROM NOTICE WHERE ADMINID LIKE '%' || TRIM(' 작성자 ') || '%' ORDER BY NOTICEDATE DESC) N)
  WHERE RN BETWEEN 2 AND 3;
-- 작성자검색 갯수 (페이징)
SELECT COUNT(*) FROM NOTICE WHERE ADMINID LIKE '%' || TRIM(' 작성자 ') || '%';
...
-- 상품상세에서 해당 상품의 문의글 리스트 (PRODUCTCODE)
SELECT * FROM
  (SELECT ROWNUM RN, A.* FROM
    (SELECT Q.*,MEMBERNAME FROM QNA Q, MEMBER M 
      WHERE Q.MEMBERID = M.MEMBERID 
        AND PRODUCTCODE = 'P0001' 
        ORDER BY QNADATE DESC) A)
  WHERE RN BETWEEN 1 AND 10;
-- 상품상세에서 해당상품의 문의글 갯수
SELECT COUNT(*) FROM QNA WHERE PRODUCTCODE = 'P0001';



-- 상품상세에서 해당 상품의 리뷰글 리스트 (PRODUCTCODE)
SELECT * FROM
  (SELECT ROWNUM RN, A.* FROM
  ...
  WHERE RN BETWEEN 1 AND 10;
-- 상품상세에서 해당상품의 리뷰글 갯수
SELECT COUNT(*) FROM REVIEW WHERE PRODUCTCODE = 'P0001';
...
INSERT INTO COUPON (COUPONNUM, MEMBERID, COUPONNAME, COUPONDISCOUNT)
  VALUES (COUPON_SEQ.NEXTVAL, 'aaa', '회원가입쿠폰', 2000);
