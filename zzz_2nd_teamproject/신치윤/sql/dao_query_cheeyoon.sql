...
-- 장바구니에 새 상품 등록 (memberId, productCode, qty, sizeNum, colorNum)
...
-- 장바구니 상품 수량 업데이트 (memberId, productCode, qty, sizeNum, colorNum)
UPDATE CART 
  SET QTY = 3,
...
select * from cart;
SELECT A.*
  FROM (SELECT C.*, TYPE, IMAGE, PRODUCTNAME, PRODUCTPRICE, PRODUCTDISCOUNT, PRODUCTSIZE, PRODUCTCOLOR  FROM CART C, PRODUCT P, IMAGE I, SIZES S, COLOR CO 
  WHERE C.PRODUCTCODE = P.PRODUCTCODE AND P.PRODUCTCODE = I.PRODUCTCODE AND TYPE = 'title' AND C.SIZENUM = S.SIZENUM AND C.COLORNUM = CO.COLORNUM AND C.PRODUCTCODE = 'P0001') A;
