...
INSERT INTO MEMBER (MEMBERID, MEMBERPW, MEMBERNAME, MEMBEREMAIL, MEMBERTEL, MEMBERPOST, memberADDR1, memberADDR2, memberBIRTH)
    VALUES ('bbb', '111', '김길동', 'tiffany456@naver.com', '010-5468-3213', '02165','서울시 서대문구 신촌동 신촌이젠아카데미','301호','90-11-08');
INSERT INTO MEMBER (MEMBERID, MEMBERPW, MEMBERNAME, MEMBEREMAIL, MEMBERTEL, MEMBERPOST, MEMBERADDR1, MEMBERADDR2, MEMBERBIRTH)
...
INSERT INTO MEMBER (MEMBERID, MEMBERPW, MEMBERNAME, MEMBEREMAIL, MEMBERTEL, MEMBERPOST, MEMBERADDR1, MEMBERADDR2, MEMBERBIRTH)
    ...
    INSERT INTO MEMBER (MEMBERID, MEMBERPW, MEMBERNAME, MEMBEREMAIL, MEMBERTEL, MEMBERPOST, MEMBERADDR1, MEMBERADDR2, MEMBERBIRTH)
    VALUES ('rrr', '111', '김소연', '@naver.com', '010-5468-3213', null, null, null, '');
SELECT * FROM MEMBER;
commit;
...
SELECT SUM((PRODUCTPRICE*(PRODUCTDISCOUNT * 0.01)*NVL(QTY,0)))DISCOUNT 
			FROM ORDERDETAIL OD, PRODUCT P WHERE OD.PRODUCTCODE = P.PRODUCTCODE AND ORDERCODE = '2305060002';

select sum(cost) from ORDERDETAIL where ordercode='2305060007';
...
SELECT *
    ...
    FROM (SELECT A.*
    FROM (SELECT R.*,CATEGORY,IMAGE, MEMBERNAME, PRODUCTNAME FROM REVIEW R, PRODUCT P, IMAGE I, MEMBER M WHERE P.PRODUCTCODE = R.PRODUCTCODE AND R.MEMBERID = M.MEMBERID AND I.PRODUCTCODE=P.PRODUCTCODE AND TYPE='title' ORDER BY REVIEWDATE DESC)A
        WHERE (REVIEWTITLE LIKE '%'||TRIM('강')||'%' OR REVIEWCONTENT LIKE '%'||TRIM('강')||'%'))B)
        WHERE RN BETWEEN 1 AND 20;

-- 카테고리가 패션중에서 리뷰제목이나 리뷰 내용을 검색했을 때 나오는 목록 (최신순) fashion
SELECT *
    FROM (SELECT ROWNUM RN, B.*
    FROM (SELECT A.*
    FROM (SELECT R.*,CATEGORY, MEMBERNAME FROM REVIEW R, PRODUCT P, MEMBER M WHERE P.PRODUCTCODE = R.PRODUCTCODE AND R.MEMBERID = M.MEMBERID ORDER BY REVIEWDATE DESC)A
        WHERE (REVIEWTITLE LIKE '%'||TRIM('리뷰제목')||'%' or REVIEWCONTENT LIKE '%'||TRIM('리뷰제목')||'%'))B 
            WHERE CATEGORY = 'fashion')
        WHERE RN BETWEEN 1 AND 20;

-- 카테고리가 장난감중에서 리뷰제목이나 리뷰 내용을 검색했을 때 나오는 목록 (최신순) toy
SELECT *
    FROM (SELECT ROWNUM RN, B.*
    FROM (SELECT A.*
    FROM (SELECT R.*,CATEGORY, MEMBERNAME FROM REVIEW R, PRODUCT P, MEMBER M WHERE P.PRODUCTCODE = R.PRODUCTCODE AND R.MEMBERID = M.MEMBERID ORDER BY REVIEWDATE DESC)A
        WHERE (REVIEWTITLE LIKE '%'||TRIM('')||'%' AND REVIEWCONTENT LIKE '%'||TRIM('')||'%'))B 
            WHERE CATEGORY = 'toy')
        WHERE RN BETWEEN 1 AND 20;
-- 카테고리가 음식중에서 리뷰제목이나 리뷰 내용을 검색했을 때 나오는 목록 (최신순) food
SELECT *
    FROM (SELECT ROWNUM RN, B.*
    FROM (SELECT A.*
    FROM (SELECT R.*,CATEGORY FROM REVIEW R, PRODUCT P WHERE P.PRODUCTCODE = R.PRODUCTCODE ORDER BY REVIEWDATE DESC)A
        WHERE (REVIEWTITLE LIKE '%'||TRIM('')||'%' AND REVIEWCONTENT LIKE '%'||TRIM('')||'%'))B 
            WHERE CATEGORY = 'food')
        WHERE RN BETWEEN 1 AND 20;
-- 카테고리가 산책중에서 리뷰제목이나 리뷰 내용을 검색했을 때 나오는 목록 (최신순)walking
SELECT *
    FROM (SELECT ROWNUM RN, B.*
    FROM (SELECT A.*
    FROM (SELECT R.*,CATEGORY, MEMBERNAME FROM REVIEW R, PRODUCT P, MEMBER M WHERE P.PRODUCTCODE = R.PRODUCTCODE AND R.MEMBERID = M.MEMBERID ORDER BY REVIEWDATE DESC)A
        ...
        WHERE RN BETWEEN 1 AND 20;
-- 카테고리가 목욕중에서 리뷰제목이나 리뷰 내용을 검색했을 때 나오는 목록 (최신순) clean
SELECT *
    FROM (SELECT ROWNUM RN, B.*
    FROM (SELECT A.*
    FROM (SELECT R.*,CATEGORY, MEMBERNAME FROM REVIEW R, PRODUCT P, MEMBER M WHERE P.PRODUCTCODE = R.PRODUCTCODE AND R.MEMBERID = M.MEMBERID ORDER BY REVIEWDATE DESC)A
...
SELECT count(*)
    FROM (SELECT A.*
    FROM (SELECT R.*,CATEGORY, MEMBERNAME FROM REVIEW R, PRODUCT P, MEMBER M WHERE P.PRODUCTCODE = R.PRODUCTCODE AND R.MEMBERID = M.MEMBERID ORDER BY REVIEWDATE DESC)A
        WHERE (REVIEWTITLE LIKE '%'||TRIM('')||'%' AND REVIEWCONTENT LIKE '%'||TRIM('')||'%'));
-- 패션        
SELECT count(*)
    FROM (SELECT A.*
    FROM (SELECT R.*,CATEGORY, MEMBERNAME FROM REVIEW R, PRODUCT P, MEMBER M WHERE P.PRODUCTCODE = R.PRODUCTCODE AND R.MEMBERID = M.MEMBERID ORDER BY REVIEWDATE DESC)A
...
SELECT *
    FROM (SELECT ROWNUM RN, A.*
        FROM (SELECT R.*,(SELECT COUNT(*) FROM GOOD G WHERE G.REVIEWNUM = R.REVIEWNUM) GOODCOUNT FROM REVIEW R ORDER BY GOODCOUNT DESC) A)
    WHERE RN BETWEEN 1 AND 3;
    select * from review r, good g where r.reviewNum = g.reviewNum;
-- 좋아요 관련 dao
    -- 리뷰글에 좋아요 한 총 갯수
...
-- 리뷰글에 댓글 갯수
SELECT COUNT(*)CNT FROM COMMENTS WHERE REVIEWNUM=1;

-- 리뷰글에 댓글 리스트 
SELECT C.* FROM COMMENTS C, REVIEW R WHERE C.REVIEWNUM=R.REVIEWNUM ORDER BY COMMENTDATE DESC;
  -- 최종  
SELECT * 
    FROM (SELECT ROWNUM RN, C.*
        FROM (SELECT C.* FROM COMMENTS C, REVIEW R WHERE C.REVIEWNUM=R.REVIEWNUM AND R.REVIEWNUM=1 ORDER BY COMMENTDATE DESC)C)
    WHERE RN BETWEEN 1 AND 10;
...
INSERT INTO REVIEW (REVIEWNUM, MEMBERID, PRODUCTCODE, REVIEWTITLE, REVIEWCONTENT, REVIEWSCORE, REVIEWIMAGE, REVIEWIP)
  ...
  '리뷰제목', '아주 잘 먹어요.', 0, 'review.jpg', '192.168.0.1');
SELECT * FROM REVIEW;
-- 리뷰 글 상세보기 전 작업 (글 봤을 때 조회 수 올리기)
UPDATE REVIEW SET REVIEWHIT = REVIEWHIT + 1 WHERE REVIEWNUM=1;
-- 리뷰 글 상세보기
SELECT * FROM REVIEW WHERE REVIEWNUM = 1;
SELECT R.*, IMAGE, PRODUCTNAME FROM REVIEW R, PRODUCT P, IMAGE I WHERE R.PRODUCTCODE=P.PRODUCTCODE AND P.PRODUCTCODE=I.PRODUCTCODE AND TYPE='title' AND REVIEWNUM=40;
-- 리뷰 글 수정
UPDATE REVIEW SET 
        REVIEWTITLE = '수정합니다',
        REVIEWCONTENT = '수정본문',
...
SELECT * FROM REVIEW;
-- 리뷰 글 삭제
DELETE FROM COMMENTS WHERE COMMENTNUM=1;
-- 리뷰 글에 댓글 작성 전 작업
...
INSERT INTO COMMENTS (COMMENTNUM, MEMBERID, REVIEWNUM, COMMENTCONTENT, COMMENTIP)
  VALUES (COMMENTS_SEQ.NEXTVAL, 'aaa', 
        ...
        '댓글에 댓글답변','192.168.0.23');

SELECT * FROM COMMENTS WHERE COMMENTNUM=1;
commit;
-- 댓글 답변 확인용
SELECT MEMBERID, 
...
SELECT R.*, PRODUCTNAME FROM REVIEW R, IMAGE I, PRODUCT P WHERE R.PRODUCTCODE = P.PRODUCTCODE AND P.PRODUCTCODE = I.PRODUCTCODE AND MEMBERID='aaa';

SELECT * FROM MEMBER M, COUPON C WHERE M.MEMBERID = C.MEMBERID AND M.MEMBERID='bbb';
...
INSERT INTO REVIEW (REVIEWNUM, MEMBERID, PRODUCTCODE, REVIEWTITLE, REVIEWCONTENT, REVIEWSCORE, REVIEWIMAGE, REVIEWIP)
  VALUES (REVIEW_SEQ.NEXTVAL, 'aaa', 
  (SELECT PRODUCTCODE FROM PRODUCT WHERE PRODUCTNAME LIKE '%' || '개목줄' || '%'), 
  '리뷰제목', '강아지가 켁켁거리지 않아요.', 2, 'n1.jpeg', '192.168.0.1');
  

  
-- 리뷰 작성시 보여질 상품명, 상품이미지 
SELECT R.*, IMAGE, PRODUCTNAME, PRODUCTPRICE, MEMBERNAME FROM REVIEW R, PRODUCT P, IMAGE I, MEMBER M WHERE R.PRODUCTCODE=P.PRODUCTCODE AND P.PRODUCTCODE=I.PRODUCTCODE AND M.MEMBERID=R.MEMBERID AND TYPE='title' AND REVIEWNUM = 2;
