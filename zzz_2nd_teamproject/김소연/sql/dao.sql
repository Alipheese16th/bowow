...
SELECT GRADE FROM MEMBER M, MEMBERGRADE G WHERE M.GRADENO = G.GRADENO AND MEMBERID = 'aaa'; -- bbb 등급
  ...
  -- 나의 주문내역
    SELECT * 
        FROM (SELECT ROWNUM RN, A.*
            FROM (SELECT O.*, PRODUCTNAME, IMAGE, ORDERDATE FROM PRODUCT P, ORDERDETAIL O, IMAGE I, ORDERS OS WHERE P.PRODUCTCODE = O.PRODUCTCODE AND P.PRODUCTCODE = I.PRODUCTCODE AND TYPE='title' AND O.ORDERCODE = OS.ORDERCODE AND MEMBERID = 'aaa' ORDER BY ORDERDATE DESC)A)
        WHERE RN BETWEEN 1 AND 2;    
