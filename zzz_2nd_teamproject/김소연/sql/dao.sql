...
-- 상세보기 orders 정보
SELECT * FROM ORDERS WHERE ORDERCODE = '2305020015';
select * from orderdetail;
SELECT * FROM ORDERS;
...
SELECT *
    FROM (SELECT ROWNUM RN, R.*
        FROM (SELECT * FROM REVIEW ORDER BY REVIEWHIT DESC)R)
        WHERE RN BETWEEN 1 AND 3;

-- 리뷰 글 목록 카테고리별 (검색) 패션
SELECT *
    FROM (SELECT ROWNUM RN, A.*
        FROM (SELECT R.* FROM REVIEW R, PRODUCT P WHERE R.PRODUCTCODE = P.PRODUCTCODE AND REVIEWTITLE LIKE '%'||TRIM('리뷰제목')||'%' OR REVIEWCONTENT LIKE '%'||TRIM('이 티셔츠 이상해요')||'%' AND CATEGORY IN (select category from product where category='toy') ORDER BY REVIEWHIT DESC)A)
        WHERE RN BETWEEN 1 AND 3;

-- 리뷰 글 목록 카테고리별 (검색) 장난감 
SELECT *
    FROM (SELECT ROWNUM RN, A.*
        FROM (SELECT R.*, P.* FROM REVIEW R, PRODUCT P WHERE R.PRODUCTCODE = P.PRODUCTCODE AND REVIEWTITLE LIKE '%'||TRIM('리뷰제목')||'%' OR REVIEWCONTENT LIKE '%'||TRIM('이 티셔츠 이상해요')||'%' and CATEGORY = 'fashion' ORDER BY REVIEWHIT DESC)A)
        WHERE RN BETWEEN 1 AND 10;
